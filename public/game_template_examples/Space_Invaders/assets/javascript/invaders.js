...
function enemyHitsPlayer(player,bullet){
    ...
    if (live){
        SGAME.triggerLO(1,function(pass){
            if(pass){
                rewardLabel = game.add.text(10, h-20, "You have been rewarded with a new life", { font: '16px Arial', fill: '#fff' });
                setTimeout(function(){
                    if(typeof rewardLabel != "undefined"){
                        rewardLabel.destroy();
                    }
                },1900);
            } else {
...
function togglePause(){
    if(game.paused === false){
        game.paused = true;
        choiseLabel = game.add.text(w/2, h/2, 'Pause', { font: '60px Arial', fill: '#fff' });
        choiseLabel.anchor.setTo(0.5, 0.5);
    } else {
        if(typeof choiseLabel != "undefined"){
          choiseLabel.destroy();  
        }
        game.paused = false;
    }
