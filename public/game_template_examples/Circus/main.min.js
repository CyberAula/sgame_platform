        ...
        update: function() {
            !this.enterPressed && this.game.input.keyboard.isDown(Phaser.Keyboard.ENTER) && (this.enterPressed = !0, this.blinkedTimes = -1, this.timerBlinker = setInterval(function(a) {
                a.blinkedTimes++, a.blinkedTimes > 10 && (clearInterval(a.timerBlinker), GameCtrl.data = {
                    ...
                    nextState: "Stage01" //Change initial level here
...
function() {
    GameCtrl.Prestage = function() {}, GameCtrl.Prestage.prototype = {
        create: function () {
            this.add.text(this.game.width/2-100, this.game.world.centerY-50, GameCtrl.data.textToRender, {
                    font : '48px "arcadeclasic"',
                    fill : '#fff',
                    align : 'left'
                });
        
            if(typeof GameCtrl.data.nextState === "undefined"){
                return;
            }
...
function() {
    GameCtrl.Stage01 = function() {}, GameCtrl.Stage01.prototype = {
        ...
        triggerGameover: function(){
            //console.log("triggerGameOver Stage 01");
            if(this.sgame_flag === true){
                return;
            }
            this.sgame_flag = true;

            var togglePause = SGAME.losCanBeShown(1);
            ...
            SGAME.triggerLO(1,function(pass){
                if(pass){
                    //Continue playing
                    setTimeout(function(){
                        that.sgame_flag = false;
                    },1750);
                    if(togglePause){
                        that.cursors.up.reset();
                        that.cursors.down.reset();
                        that.cursors.left.reset();
                        that.cursors.right.reset();
                        that.music.play();
                        that.game._paused = false;
                    }
                } else {
                    if(togglePause){
                        that.game._paused = false;
                    }
                    that._triggerGameover();
                }
            });  
        },
...
function() {
    GameCtrl.Stage02 = function() {}, GameCtrl.Stage02.prototype = {
        ...
        create: function() {
            this.gameover = !1, this.music = this.add.audio("stage2"), this.music.play(), this.cursors = this.game.input.keyboard.createCursorKeys(), this.world.setBounds(0, 0, 8192, 200), this.background = this.add.tileSprite(0, 200, 8192, 552, "stage02"), this._createMeters(), this._createPlayer(), this._createObstacles(), this.floor = this.game.add.sprite(0, 417); 
            // this.endStage=this.game.add.sprite(300, 300, "clown",'endLevel1');
            this.endStage=this.game.add.sprite(7892, 300, "clown",'endLevel1');
            this.physics.enable(this.floor, Phaser.Physics.ARCADE), this.physics.enable(this.endStage, Phaser.Physics.ARCADE), this.endStage.scale.x = 3, this.endStage.scale.y = 3, this.endStage.body.immovable = !0, this.endStage.body.collideWorldBounds = !0, this.floor.body.immovable = !0, this.floor.body.collideWorldBounds = !0, this.floor.body.width = this.game.world.width, this.recicleFireCirclesWall = this.game.add.sprite(-12, 600), this.physics.enable(this.recicleFireCirclesWall, Phaser.Physics.ARCADE), this.recicleFireCirclesWall.body.immovable = !0, this.recicleFireCirclesWall.body.height = 500, this.recicleFireCirclesWall.body.width = 2
        },
        triggerGameover: function(){
            //console.log("triggerGameOver Stage 02");
            if(this.sgame_flag === true){
                return;
            }
            this.sgame_flag = true;

            var togglePause = SGAME.losCanBeShown(2);
            ...
            SGAME.triggerLO(2,function(pass){
                if(pass){
                    //Remove close monkeys
                    var monkeysToRemove = [];
                    that.monkeys.forEach(function(m){
                        var mdistance = Math.pow(Math.abs(this.player.position.x-m.position.x),2) + Math.pow(Math.abs(this.player.position.y-m.position.y),2);
                        if(mdistance < 8500){
                            monkeysToRemove.push(m);
                        }
                    }, that);

                    for(var i=0; i<monkeysToRemove.length;i++){
                        that._monkyOut(monkeysToRemove[i]);
                    }
        ...
        triggerWin: function(){
            GameCtrl.data={textToRender:'GAME OVER' };
        ...
        update: function () {
            ...
            this.game.physics.arcade.collide(this.player, this.endStage, this.triggerWin, null, this);
...
// var game = new Phaser.Game(1024, 768, Phaser.AUTO, "game", !1, !1),
