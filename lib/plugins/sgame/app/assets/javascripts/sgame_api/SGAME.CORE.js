SGAME.CORE = (function(){
	...
	var _nlosuccess = 0; //Number of LOs successfully consumed
	var _nsuccess = 0; //Total number of times a LO has been successfully consumed
	var _final_screen_shown = false;
	var _lastLoTimeStamp = undefined; //Timestamp of the last time a LO was shown
	//Internal vars for sequencing
	...
	var _loadSettings = function(settings){
		...
		if(typeof _settings["los"] === "object"){
			...
			for(var i=0; i<_nLOs;i++){
				...
				_settings["los"][lo_ids[i]]["nsuccess"] = 0; //number of times successfully consumed
		...
		if(_sequence_enabled === true){
			...
			//Delete LOs without group
			//This could be disabled based on game settings to show non-grouped LOs after the sequence is finished
	...
	var showLO = function(lo,callback){
		...
		SGAME.Fancybox.create({lo: lo}, function(report){
			...
			if((_sequence_enabled === true)&&(_sequence_finished === false)){
				//Check if the groups to which the LO belongs have been "shown" or "successfully consumed" and if other groups should be unlocked or locked
	...
	var _updateFlagsAndTracking = function(){
		...
		if(_sequence_enabled === true){
			if(_sequence_finished === true){
				//When a sequence is used and has finished, only those LOs that have been shown should be taken into account for calculating progress measure and score
	...
	var _shouldShowFinalScreen = function(){
		...
		if((_sequence_enabled === true)&&(_sequence_finished === true)){
			//When sequencing is used and the sequence has finished, only those LOs that have been shown should be taken into account
	...
	/*
	 * This strategy is applied by default and randomly chooses one learning object among the ones that have been shown less times.
	 */
			...
			if(group["can_be_shown"]===true){
				for(var j=0; j<group.los.length; j++){
					if(lo_candidates_mapping_ids.indexOf(group.los[j])!==-1){
						//loCandidatesFromMapping only includes LOs that can be shown ((_settings["los"][group.los[j]]["can_be_shown"]===true))
	...
	//Wait for SGAME to call loadSettings
